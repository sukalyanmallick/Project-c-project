...
TcpServer::TcpServer(AiEngine& engine, View::ServerLogger* logger)
    : engine_(engine),
      logger_(logger),
...
bool TcpServer::initialize() {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        logger_->logError("WSAStartup failed.");
        return false;
    }

    serverSocket_ = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (serverSocket_ == INVALID_SOCKET) {
        logger_->logError("socket() failed.");
        WSACleanup();
        return false;
    }
    ...
    if (listen(serverSocket_, SERVER_LISTEN_BACKLOG) == SOCKET_ERROR) {
        logger_->logError("listen() failed.");
        closesocket(serverSocket_);
        WSACleanup();
        return false;
    }
...
bool TcpServer::waitForClient() {
    logger_->logInfo("Waiting for a client...");
    ...
    logger_->logClientConnected(inet_ntoa(clientAddr.sin_addr));
    return true;
}
...
void TcpServer::handleClientSession() {
    ...
    while (true) {
        ...
        logger_->logMessageReceived(incomingMessage);
        ...
        if (send(clientSocket_,
                 ...
                 0) == SOCKET_ERROR) {
            logger_->logError("send() failed.");
            break;
        }

        if (sessionShouldExit(incomingMessage)) {
            logger_->logInfo("Session ended by client.");
            break;
        }
    }
}
...
void TcpServer::shutdown() {
    ...
    WSACleanup();
    logger_->logInfo("Shutdown complete.");
}
